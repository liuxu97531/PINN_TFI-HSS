...
import geo_utils.geo_obj as gd2
import sympy
...
from PINN.net import Net
from PINN.models import PDE, is_neumann_boundary_x, is_neumann_boundary_y, PDE_inverse, setup_seed
from PINN.sample_latin import sample_latin
from PINN.sample_uniform import sample_uniform
from PINN.sample import obs_sample
...
def train(args, Epoch, num, x_obs, y_obs, data_weight, sample):
    ...
    for epoch in range(Epoch):
        ...
        # boundary
        point_bc_ostile = rectan.sample_boundary(n_b_bc, Eq(YS, 0) & ((XS > 4.5) & (XS < 5.5)))
        point_bc_down = rectan.sample_boundary(n_b_bc, Eq(YS, 0) & ((XS < 4.5) | (XS > 5.5)))
        point_bc_left_right = rectan.sample_boundary(n_b_bc, Eq(XS, 0) | Eq(XS, 10))
        point_bc_up = rectan.sample_boundary(n_b_bc, Eq(YS, 10))
...
if __name__ == '__main__':
    ...
    # comparsion with three sample method
